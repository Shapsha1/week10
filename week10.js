...
const printMessage = (message = "Сообщение отсутствует") => {
    console.log(message);
};
printMessage();
...
function thirdFunction(f, g) {
    if (typeof f === "number" && typeof g === "undefined") {
        g = f;
    } else if (typeof g === "number" && typeof f === "undefined") {
        f = g;
    } else if (typeof g === "undefined" && typeof f === "undefined") {
        f = 2;
        g = 3;
    }
    return f * g;
...
const greeting = (guestName = "Гость") => {
...
rabbit = "имя кролика";
const findAnimal = (cat = "имя кошки") => {
...
function calculateYearsSinceLastOlympics(thisYear, lastOlympicsYear = 2021) {
    return thisYear - lastOlympicsYear;
}

console.log("С момента летней олимпиады в Токио прошло " + calculateYearsSinceLastOlympics(2023) + " года");
...
function calculateAge(thisYear, yearOfBirth) {
    return thisYear - yearOfBirth;
}

console.log(calculateAge(thisYear, friendYearOfBirth));
//Задание 17
...
const calculateYourAge = (yearOfBirth) => {
    let yourAge = thisYear - yearOfBirth;
    return yourAge;
};

console.log(calculateYourAge(2002));
//Задание 18
...
const raiseNumber = (number, raise) => {
    let result = number ** raise;
